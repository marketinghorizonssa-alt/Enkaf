...
function hero_image_for_page(array $page): string {
    return match ((string)($page['theme'] ?? 'home')) {
        'corporate' => '/assets/img/office/hero-corporate.webp',
        'disputes' => '/assets/img/office/hero-disputes.webp',
        'ip' => '/assets/img/office/hero-ip.webp',
        'realestate' => '/assets/img/office/hero-realestate.webp',
        default => '/assets/img/office/hero-home.webp',
    };
}

function hero_alt_for_page(array $page): string {
    return match ((string)($page['theme'] ?? 'home')) {
        'corporate' => 'محامٍ من فريق إنكاف في مكتب جدة',
        'disputes' => 'عمل قانوني ومراجعة مستندات في إنكاف',
        'ip' => 'مكتب إنكاف في جدة',
        'realestate' => 'مكتب إنكاف في جدة وإطلالة المدينة',
        default => 'مكتب إنكاف للمحاماة والاستشارات القانونية في جدة',
    };
}
...
function trust_strip(): string {
    $items = [
        ...
        ['building','مكتب جدة','اجتماعات حضورية بخصوصية عالية'],
        ['document','مرونة رقمية','اجتماعات ومتابعة عن بُعد'],
        ['shield','أفراد وشركات','خدمة قانونية حسب طبيعة الملف'],
    ];
...
function process_section(): string {
    $steps = [
        ['01','نبدأ من المطلوب','ترسل بيانات التواصل ونوع الخدمة، بدون تفاصيل مرهقة في البداية.'],
        ['02','الفريق يراجع الملف','نحدد المحامي المناسب وما نحتاجه من معلومات أو مستندات.'],
...
function office_experience_section(): string {
    return '<section class="office-story"><div class="container office-story-grid">'
        . '<figure class="office-photo office-photo-main"><img src="/assets/img/office/office-lounge.webp" width="1600" height="1200" loading="lazy" decoding="async" alt="مكتب إنكاف في جدة"></figure>'
        . '<div class="office-story-copy"><span class="section-label">من جدة</span><h2>مكتب قانوني بهدوء يليق بالقرارات المهمة.</h2><p>صممنا تجربة إنكاف لتكون واضحة ومريحة من أول تواصل: استقبال هادئ، خصوصية في الاجتماعات، وفريق يتعامل مع الملف بجدية من غير تعقيد في التواصل.</p><div class="office-facts"><span>هوية سعودية</span><span>خصوصية في الاجتماعات</span><span>تجربة مهنية من أول زيارة</span></div></div>'
        . '</div></section>';
}

function team_section(): string {
    return '<section class="team-section"><div class="container team-grid">'
        . '<div class="team-copy"><span class="section-label light">فريق إنكاف</span><h2>الملف يذهب إلى التخصص المناسب، لا إلى قالب واحد.</h2><p>يضم المكتب أكثر من محامٍ، ويُوزع العمل بحسب نوع القضية أو المعاملة. الهدف أن يصل الملف إلى الخبرة الأقرب له، مع بقاء التواصل منظمًا وواضحًا.</p><a class="text-link" href="#form">تواصل مع الفريق' . icon_svg('arrow') . '</a></div>'
        . '<figure class="team-photo"><img src="/assets/img/office/team-work.webp" width="1600" height="900" loading="lazy" decoding="async" alt="محامٍ من فريق إنكاف أثناء العمل"></figure>'
        . '</div></section>';
}

function digital_section(): string {
    return '<section class="digital-section"><div class="container digital-grid">'
        . '<figure class="digital-photo"><img src="/assets/img/office/digital-office.webp" width="788" height="1400" loading="lazy" decoding="async" alt="مساحة عمل رقمية في مكتب إنكاف"></figure>'
        . '<div class="digital-copy"><span class="section-label">مرونة في التواصل</span><h2>قريبون منك، حتى لو لم تكن في المكتب.</h2><p>يمكن ترتيب الاجتماعات عن بُعد، ومراجعة المستندات ومتابعة التواصل رقميًا عندما يكون ذلك أنسب لك وللفريق.</p><div class="digital-list"><div><strong>اجتماعات مرئية</strong><span>للنقاش والمتابعة بدون زيارة في كل مرة.</span></div><div><strong>تبادل المستندات</strong><span>تنظيم الملفات والمراجعات بطريقة أسهل وأسرع.</span></div><div><strong>متابعة مرنة</strong><span>حضوري أو رقمي حسب طبيعة الطلب.</span></div></div></div>'
        . '</div></section>';
}
...
function page_html(array $page): string {
    ...
    $body = '<body class="theme-' . e($page['theme']) . '">' . gtm_body_fallback() . header_html()
        . '<main><section class="hero"><div class="hero-media" aria-hidden="true"><img src="' . e($hero) . '" width="1920" height="1080" fetchpriority="high" decoding="async" alt=""></div><div class="hero-shade" aria-hidden="true"></div><div class="container hero-grid"><div class="hero-copy"><span class="eyebrow">' . e($page['eyebrow']) . '</span><h1>' . e($page['h1']) . '</h1><p class="hero-intro">' . e($page['intro']) . '</p><div class="hero-chips">' . $chips . '</div><div class="hero-meta"><span>جدة</span><span>حضوري وعن بُعد</span><span>للأفراد والشركات</span></div></div>' . form_html($page) . '</div></section>'
        . trust_strip()
